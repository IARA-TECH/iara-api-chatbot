...
async def respond(session_id:UUID, user_message:str, user_id:UUID) -> models.RespondData:
    session = await Session.find_one(Session.id==session_id)
    if (not session):
    ...
    chat = Chat(user_message=user_message, response=response, agent_id=agent_id, sent_at=datetime.now(), total_tokens=total_tokens)
