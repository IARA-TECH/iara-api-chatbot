...
async def create_session(
    body: models.RespondInput, current_user: UserAccount = Depends(get_current_user)
) -> models.RespondResponse:
    response = await usecases.respond(
        body.session_id, body.user_message, current_user.pk_uuid
    )
